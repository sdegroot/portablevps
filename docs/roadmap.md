...
**Why it fits:** every host already runs node_exporter (the monitoring plane),
and we already publish custom metrics through node_exporter's textfile
collector (the backup outcome metrics). Mesh RTT is the same pattern.

**Design sketch (lightweight first cut):**
- A small systemd timer on each host pings every other portablevps peer's
  NetBird IP and writes textfile-collector metrics, e.g.
...
- The separate monitoring server scrapes node_exporter and gets per-peer RTT
  for free; alert on RTT above a threshold or `mesh_up == 0` (partition).

**Richer alternative:** run `prometheus/blackbox_exporter` with ICMP probes to
peer mesh IPs instead of the textfile timer — gives probe success/duration and
is the more standard building block; heavier. Smokeping-style history is a
third option.
